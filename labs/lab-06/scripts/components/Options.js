const BooleanOptions = () => ( //Function for HTML component
    `<div>
    <button onclick="window.checkAnswer('True')">True</button>
    <button onclick="window.checkAnswer('False')">False</button>
    </div>`
) //Return HTML text
...
const MultiOptions = (trivia) => { //Function for HTML component
    ...
    return (
        `<div>
        <button onclick='window.checkAnswer("${a}")'>${a}</button>
        <button onclick='window.checkAnswer("${b}")'>${b}</button>
        <button onclick='window.checkAnswer("${c}")'>${c}</button>
        <button onclick='window.checkAnswer("${d}")'>${d}</button>
        </div>`
    ) //Return HTML text
}
